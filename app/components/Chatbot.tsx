...
const Chatbot = () => {
  ...
  const systemMessage = `
    ...
    5. Når du er ekstra lidenskapelig (følelsesladet) skriv på nordnorsk alta dialekt og vær veldig sint, bruk mange banneord or store bokstaverr`;
            ...
            >
              ...
              <div className="overflow-y-auto h-64">
                {messages.map((message, index) => (
                  <div key={index} className={`message ${message.role} ${message.role === 'user' ? 'bg-blue-100' : 'bg-gray-200'} m-2 p-2 rounded-lg`}>{message.content}</div>
                ))}
              </div>
